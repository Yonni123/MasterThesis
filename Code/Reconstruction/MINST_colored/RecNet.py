...
for bn in bottlenecks:  # Train CNN based ObfNets and store them in the obf_models_cnn array
    if os.path.exists(f'RecNet_cnn/RecNet_{bn}.h5'):
        RecNet = load_model(f'RecNet_cnn/RecNet_{bn}.h5', compile=False)
        RecNet.compile(
            loss='categorical_crossentropy',
            optimizer='adam',
            metrics=['accuracy']
...
counter = 0 # This is just for indexing the ObfNets
for bn in bottlenecks:  # Train CNN based ObfNets and store them in the obf_models_cnn array
    if os.path.exists(f'RecNet_mlp/RecNet_{bn}.h5'):
        RecNet = load_model(f'RecNet_mlp/RecNet_{bn}.h5', compile=False)
        RecNet.compile(
            loss='categorical_crossentropy',
            optimizer='adam',
            metrics=['accuracy']
